...
class BrainAgeLoss(nn.Module):
    ...
    def __init__(self, alpha=1.0, beta=0.3, gamma=0.2, eps=1e-8, 
                 smoothing=0.1, use_huber=False, delta=1.0):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
    ...
    def forward(self, pred, target, demographic_factors=None):
        """
        ...
        if self.gamma > 0:
            ...
            # Older subjects are typically under-represented in most datasets
        ...
        # 4. Bias regularization (prevent prediction toward mean)
...
# --- DropPath Module (Stochastic Depth) ---
...
# --- Hybrid Embedding (CNN stem) for 3D inputs ---
